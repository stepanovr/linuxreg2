...
{
//  uint64_t read_result = *((volatile uint32_t *)v_addr);
...
void write32(void* v_addr, uint32_t value)
{
...
char* mem_access(void* p_addr, uint64_t value, int bits, int read)
{
  ...
  page = ((uint32_t)p_addr / MAP_PAGESIZE) * MAP_PAGESIZE;
  offset = (uint32_t)p_addr % MAP_PAGESIZE;
  ...
  if(!read)
  {
    ...
    switch(bits)
    {
      ...
      case 32:
        write32(v_addr, (uint32_t)value);
        break;

      case 64:
        write32(v_addr, (uint32_t)value);
        write32(v_addr+4, (uint32_t)(value>>32));
        break;
